...
// UserQueryResult 用户信息查询结果返回表
type UserQueryResult struct {
	Nickname string `json:"nickname"`
	Sex      int    `json:"sex"`
	Email    string `json:"email"`
	Avatar   string `json:"avatar"`
	IsFriend bool   `json:"is_friend"` //是否是好友 [true 是,false-不是好友关系]
}
...
func UserQuery(c *gin.Context) {
	...
	//判断是否为好友关系
	judgeUserIsFriend, err := models.JudgeUserIsFriend(userBasic.Identity, uc.Identity)
	if err != nil {
		log.Printf("[DB ERROR]:%v\n", err)
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
...
func UserAdd(c *gin.Context) {
	...
	uc := c.MustGet("user_claims").(*helper.UserClaims)
	//判断是否为好友关系
	judgeUserIsFriend, err := models.JudgeUserIsFriend(ub.Identity, uc.Identity)
